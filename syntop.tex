...
\def\PMetApx{\mathbf{PMet}_{\mathbf{Apx}}}
...
\def\PMetApxu{\mathbf{PMet}_{\mathbf{Apx},u}}
...
\def\PMetAppl{\mathbf{PMet}_{\mathbf{App},\ell}}
\def\PMetApxl{\mathbf{PMet}_{\mathbf{Apx},\ell}}
...
\def\PMetAppm{\mathbf{PMet}_{\mathbf{App},m}}
\def\PMetApxm{\mathbf{PMet}_{\mathbf{Apx},m}}
...
\def\QGauAppm{\mathbf{QGau}_{\mathbf{App},m}}
\def\QGauApxm{\mathbf{QGau}_{\mathbf{Apx},m}}
...
\begin{defn}\label{def:pmet-maps}
  ...
  % \item $f$ is \textbf{approximal}\fxnote{This is a terrible pormanteau-pun on ``approach'' and ``proximal''; better ideas?} if for all $A\subseteq X$, $d_Y$, and $\zeta,\eta\in \Qp$, there exists a $d_X$ such that for all $z\in A$ and $y\in X$ there exists a $w\in A$ with $\min(d_Y(f(w),f(y)),\zeta) \le d_X(z,y) +\eta$.
