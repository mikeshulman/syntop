...
%   The categories of preorders, co-preorders, and comparison co-preorders are ``topological over $\mathbf{Set}$'', i.e.\ their forgetful functor to $\mathbf{Set}$ has initial lifts for all sources and (hence) final lifts for all sinks.
...
%   Similarly, given $\{f_i : X \to Y_i\}$ with each $Y_i$ a co-preorder, define $x\oapt y$ for $x,y\in X$ to mean that $f_i(x)\oapt_i f_i(y)$ for \emph{some} $i$.
...
We will use Greek letters $\al,\be,\gm$ to denote binary relations on a set, with $(x,y)\in \al$ written as $x\leapx_\al y$ or $x \oapt_\al y$ depending on the intended perspective.
