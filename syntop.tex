...
\fxwarning{Do we want this section at all?}
...
% (Wait on this until the proximity section?)
% The following notion is nontrivial even classically:
% \begin{defn}
%   Two subsets $A,B$ are \textbf{apart}, written $A\bowtie B$, if there is an $\ep>0$ such that $d(x,y)>\ep$ for all $x\in A$ and $y\in B$.
% \end{defn}

\fxwarning{Should we use that notation and name even in the non-symmetric case?  Should we postpone apartness to the section on point-point relations: order?}
