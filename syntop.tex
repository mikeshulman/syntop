...
\def\PMetTopl{\mathbf{PMet}_{\mathbf{Top},\ell}}
\def\PMetUnifl{\mathbf{PMet}_{\mathbf{Unif},\ell}}
\def\PMetProxl{\mathbf{PMet}_{\mathbf{Prox},\ell}}
\def\PMetOrdl{\mathbf{PMet}_{\mathbf{Ord},\ell}}
\def\PMetTopm{\mathbf{PMet}_{\mathbf{Top},m}}
\def\PMetUnifm{\mathbf{PMet}_{\mathbf{Unif},m}}
\def\PMetProxm{\mathbf{PMet}_{\mathbf{Prox},m}}
\def\PMetOrdm{\mathbf{PMet}_{\mathbf{Ord},m}}
\def\QGauTopm{\mathbf{QGau}_{\mathbf{Top},m}}
\def\QGauUnifm{\mathbf{QGau}_{\mathbf{Unif},m}}
\def\QGauProxm{\mathbf{QGau}_{\mathbf{Prox},m}}
\def\QGauOrdm{\mathbf{QGau}_{\mathbf{Ord},m}}
...
\section{Point-point distances: prometrics}
\label{sec:metric}
...
\subsection{Metric, gauge, and prometric spaces}
\label{sec:met-gau-pmet}
...
Symmetry and separation are really \emph{separation axioms}, and experience in general topology has shown that a better category is obtained by only imposing separation axioms when necessary rather than including them in definitions.
And allowing infinite distances is arguably of little import topologically.
Moreover, Lawvere~\cite{lawvere:metric-spaces} exhibited \eqp-metric spaces as categories enriched over $(\Rp,\ge,+)$, placing them in a wider context.\fxnote{Should we mention the enriched-multicategory approach to other topological structures?}
...
\fxnote{Should these have `anti-' on them?  Or should we distinguish by saying ``continuous in the lower sense''?}
...
To construct these, we first observe the following.
...
Note that if each $Y_i$ is a metric space \emph{and} there are finitely many of them, then $X$ has a maximal metric and hence is isomorphic in $\PMet$ to a metric space; but in general $\Met$ is not closed in $\PMet$ under initial lifts.
...
The quasi-gauge reflection of a prometric space $X$ has as gauging distances all \eqp-metrics $d$ on the set $X$ such that $d\le d'$ for some distance $d'$ in the prometric of $X$.
...
%   %, and dually if $U\in\Reu$ then $\low{U} = \setof{x\in Q | x < U}$.
...
To conclude, we survey some naturally-occurring examples.
Of course, Dedekind metric spaces are ubiquitous in mathematics and well-trusted constructively, so it is the further generalizations above that require justification by examples.

\begin{eg}
  As remarked above, $\Met$ is not closed in $\PMet$ under initial lifts, and hence not under limits.
  For instance, the product of infinitely many metric spaces is a prometric space (indeed, a gauge space), but does not naturally admit a single metric.
  Thus, Dedekind gauge spaces, at least, arise very naturally as limits of Dedekind metric spaces.
\end{eg}
...
\begin{eg}
  ...
  (This is a special case of \cref{thm:preord-pmetu}.)

  Combining this with the previous example, we find that \emph{prodiscrete spaces} are naturally upper prometric spaces (and indeed upper gauge spaces).
\end{eg}
...
\begin{eg}
  ...
  (This is a special case of \cref{thm:qapt-pmetl}.)
\end{eg}

\begin{eg}\label{eg:linf}
  If $X$ is an upper, lower, or MacNeille prometric space and $A$ is an arbitrary set, then the function set $X^A$ admits a \emph{supremum prometric} with distances defined by
  \[ d^\infty(f,g) = \sup_{a\in A} d(f(a),g(a)). \]
  where $d$ ranges over distances on $X$.
  For instance, if $X$ is a normed space with metric defined by $d(x,y) = \|x-y\|$, and we restrict to the subspace of $X^A$ consisting of functions for which $d^\infty(0,f) <\infty$, this gives the metric underlying the traditional $L^\infty$-norm.
  We emphasize that it is not Dedekind even if the original norm on $X$ was.
  Since suprema of lower reals are better-behaved than suprema of upper or MacNeille reals, usually if $X$ is Dedekind we want to consider $X^A$ as a lower prometric space.
\end{eg}

\begin{eg}\label{eg:haus}
  Classically, the \emph{Hausdorff distance} between subsets $A,B\subseteq X$ of a metric space is usually defined as the symmetrization of the distance $d(A,B) = \sup_{x\in A} \inf_{y\in B} d(x,y)$.
  Constructively, this looks suspect since it uses both suprema and infima, and for no kind of real number do we have both well-behaved suprema and infima.
  However, it is classically equivalent to:
  \[ d(A,B) = \inf \setof{ \ep>0 | \forall x\in A, \exists y\in B, d(x,y) < \ep }. \]
  This is perfectly reasonable constructively for \emph{upper} real numbers, and can be applied distance-wise to a quasi-gauge or prometric as well, producing an upper prometric on the powerset $\wp(X)$, which is only an upper prometric even if $X$ was Dedekind.
  It is not generally symmetric even if $X$ is, but can of course be symmetrized, as one usually does.
\end{eg}


\subsection{Categories of prometric spaces}
...
Formally, this problem is that $\PMet$ and its supercategories are not \emph{amnestic}, meaning that their fibers over a given set $X$ are only preorders and not partial orders.
...
In fact this solution can be extended to all the supercategories of $\PMet$ from \cref{def:pmet-maps} as well.
...
Recall that $\neg(a<b)$ is equivalent to $b\le a$ for MacNeille real numbers.
Although this fails for upper and lower real numbers in general, it is true for lower reals if $a$ is rational and for upper reals if $b$ is rational, since rational numbers satisfy trichotomy.
Therefore, taking $a=0$, we see that in any \emph{lower} prometric space we have:
\begin{equation}
  (x\leapx y) \iff \neg(x\oapt y).\label{eq:leapx-oapt}
\end{equation}
(In an upper prometric space, $\oapt$ is arguably not a reasonable thing to define.)
...
Of course, $\leapx$ is an instance of the \emph{specialization order} underlying a topology; we will return to this in \cref{sec:top-order}.
...
\begin{defn}\label{def:anti-preorder}
  ...
  \item \textbf{symmetric} if $x\oapt y$ implies $y\oapt x$.
  \item an \textbf{inequality} if it is symmetric and irreflexive.
  \item an \textbf{anti-equivalence-relation} if it is a symmetric left (hence also a right) anti-preorder.
...
\begin{thm}\label{thm:pmet-apreord}
  ...
  \end{enumerate}
  If $X$ is a lower prometric space (including MacNeille and Dedekind), then
  \begin{enumerate}[resume]
  \item The relation $\oapt$ is a quasi-apartness.
  \item If $X$ is symmetric, then $\oapt$ is an apartness relation.
...
In particular, the equality relation of a $T_0$ lower prometric space is (like that of any set with a connected irreflexive relation) \nn-stable, for $\neg\neg(x=y)$ implies $\neg(x\oapt y)\land \neg(y\oapt x)$, hence $x=y$.
...
\begin{eg}
  ...
  Note that when $Y=1$, its power set $\wp(1)$ is the set of truth values.
  Thus we also have a natural left anti-preorder on the latter set, for which $P\oapt Q$ means $P\land \neg Q$.
  Since $\top \oapt \bot$ and $(P\oapt \bot)\leftrightarrow P$ and $(\top\oapt P) \leftrightarrow \neg P$, if this is a quasi-apartness then excluded middle holds.
\end{eg}

Preorders and anti-preorders are closely related.
In fact, in classical mathematics they are entirely equivalent notions.
Constructively, we have instead an \emph{adjunction} between the following categories.
...
\begin{defn}
  ...
  \item If $X$ and $Y$ are irreflexive, then $f$ is \textbf{monotone}, or \textbf{strongly extensional}, if $f(x)\oapt f(y) \implies x\oapt y$ for all $x,y\in X$.\fxnote{``Anti-monotone'' would be natural, but is unfortunately also used for an order-reversing map.}
...
\cref{thm:ord-le-apt} gives some additional justification for defining the notion of anti-preorder using our weak transitivity axioms instead of comparison, since the negation of a preorder need not be a quasi-apartness.
However, like $\upp{(-)}:\PMetl \to \PMetu$, the functor $\neigh : \QApt \to \Preord$ does have an impredicative left adjoint, and for the same reason: its domain and codomain are both topological over $\Set$ and it preserves initial lifts.
In particular, if we apply this left adjoint to the equality relation of a set, we see that any set admits a largest quasi-apartness and similarly a largest apartness (not in general tight).
...
\begin{thm}\label{thm:macneille-preord}
  If $X$ is a lower prometric space, then its underlying preorder is obtained from its underlying anti-preorder by the functor $\neigh$ of \cref{thm:ord-le-apt}.
  In particular, it is \nn-stable.
\end{thm}
\begin{proof}
...
However, even if $X$ is Dedekind, its underlying anti-preorder is not generally the negation of its underlying preorder.
...
However, in this particular case it seems to be due mainly to the impoverished nature of a single binary relation; for more refined topological structures, the MacNeille or Dedekind condition on a prometric space will be sufficient to make the ``closeness'' and ``apartness'' structures determine each other.
...
\begin{proof}
  ...
  The remaining statements are clear from \cref{thm:up,thm:macneille-preord}.
\end{proof}
...
%   The categories of preorders, anti-preorders, and quasi-apartnesses are ``topological over $\Set$'', i.e.\ their forgetful functor to $\Set$ has initial lifts for all sources and (hence) final lifts for all sinks.
...
%   For transitivity, if $x\oapt z$, so that $f_i(x)\oapt_i f_i(z)$ for some $i$, then if $\neg(y\oapt z)$ we have $\neg (f_i(y)\oapt_i f_i(z))$, hence $f_i(x) \oapt_i f_i(y)$ and so $x\oapt y$.
...
A set of entourages is a base for \emph{some} uniformity exactly when it satisfies all of the axioms except isotony.
...
For instance, it would exclude the following examples.

\begin{eg}\label{thm:unif-ord}
  ...
  It has a right adjoint that takes the intersection of all the entourages of a quasi-uniform space; that is, we define $x\leapx y$ to mean $\forall \al, x\ent\al y$.
  Both of these functors preserve \nn-stability and symmetry.

  In particular, taking $\leapx$ to be equality, we see that every set admits a discrete uniformity, in which all reflexive relations are entourages.
  It is locally decomposable if and only if $X$ has decidable equality.
\end{eg}

We will see in \cref{sec:unif-pmet} that this is a special case of the quasi-uniformity underlying any \emph{upper} prometric, and that in general the latter is locally decomposable only when the prometric is Dedekind.
Note that local decomposability implies \nn-stability (use the same $\al$ and $\be$).

\begin{eg}\label{eg:unif-uconv}
  If $X$ is a quasi-uniform space and $A$ is any set, then every entourage $\al$ of $X$ induces a relation on the function set $X^A$ defined by
  \[ f \ent\al g \iff \forall a\in A, (f(a)\ent\al g(a)) \]
  This is a base for a quasi-uniformity on $X^A$ called the \emph{quasi-uniformity of uniform convergence}; it is symmetric if $X$ is.
  In general we cannot prove it to be locally decomposable, even if $X$ is, unless $A$ is finite (in which case it coincides with the product quasi-uniformity $\prod_{a\in A} X$).
\end{eg}

\begin{eg}\label{eg:unif-topgp}
  Let $G$ be any topological group, i.e.\ a topological space in the usual sense (see also \cref{sec:point-set}) with a continuous group structure.
  Then for each neighborhood $U$ of the identity $e\in G$, define $x\ent U y$ to mean $x\inv y\in U$.
  This is a base for a uniformity on $G$.
  In general we cannot prove it to be locally decomposable; indeed, as we will see in \cref{sec:regularity}, if it were locally decomposable then $G$ would be regular, but we cannot prove that all topological groups are regular (e.g.\ if $G$ is discrete, then it is regular iff it has decidable equality).
\end{eg}

\begin{eg}\label{eg:unif-haus}
  If $X$ is a quasi-uniform space, then for subsets $A,B\subseteq X$ and any entourage $\al$ of $X$ we define
  \[ A \ent\al B \iff \forall x\in A, \exists y\in B, x\ent\al y. \]
  This is a base for a quasi-uniformity on the powerset $\wp(X)$.
  It is not generally symmetric even if $X$ is; its symmetrization is called the \emph{Hausdorff uniformity} of a uniform space.
  In general we cannot prove it to be locally decomposable, even if $X$ is.
\end{eg}

Less well-known than uniformity is the dual notion of uniform apartness.
...
\begin{defn}
  ...
  \item a \textbf{left} (resp.\ \textbf{right}) \textbf{anti-quasi-uniform} space if
    ...
    \end{align*}
    %for any anti-entourage $\rho$, there exists an anti-entourage $\si$ such that for any $x,y,z\in X$, if $x\aent\rho z$, then $\neg(y\aent\si z)\implies (x\aent\si y)$ (resp.\ $\neg(x\aent\si y)\implies (y\aent\si z)$).  (left/right transitivity)
  \item a \textbf{quasi-uniform apartness space} if
    \fxwarning{Is this good terminology?}
    \[ \forall \rho \exists \si \forall xyz, (x\aent\rho z \implies x\aent\si y \lor y\aent\si z) \;\text{(comparison)}\]
    % for any anti-entourage $\rho$, there is an anti-entourage $\si$ such that for any $x,y,z\in X$, if $x\aent\rho z$ then either $x\aent\si y$ or $y\aent\si z$ (comparison).
  \item an \textbf{anti-uniform space} if
    $\forall\rho\exists\si\forall xy, (x\aent\rho y \implies y\aent\si x)$ (symmetry).
    %for any anti-entourage $\rho$, there is an anti-entourage $\si$ such that for any $x,y\in X$, if $x\aent\rho y$ then $y\aent\si x$ (symmetry).
...
As for examples, we will see in \cref{sec:unif-pmet} that every \emph{lower} prometric space yields a quasi-uniform apartness space.
We also have an analogue of \cref{thm:unif-ord}.
(A \textbf{base} for an anti-uniformity is a subset of basic anti-entourages such that every anti-entourage is \emph{contained in} a basic one.)

\begin{eg}\label{thm:aunif-ord}
  If $X$ is a left or right anti-preorder, then its relation $\oapt$ is a base for a left or right anti-quasi-uniformity.
  This defines two fully faithful functors $\LAPreord \to \LAQUnif$ and $\RAPreord \to \RAQUnif$, whose images consists of the (left or right) anti-quasi-uniform spaces with a largest anti-entourage.
  They have right adjoints that take the unions of all anti-entourages, i.e.\ $x\oapt y$ means $\exists \rho, x\aent\rho y$.
  Moreover, all of these functors preserve \nn-stability and symmetry, and the quasi-apartness relations correspond to the quasi-uniform apartness spaces in both directions.
  However, the anti-quasi-uniformity corresponding to an anti-preorder is only locally decomposable if the anti-preorder is decidable.
\end{eg}

\begin{eg}\label{eg:aunif-uconv}
  Dualizing \cref{eg:unif-uconv}, if $X$ is a anti-quasi-uniform space and $A$ is any set, every anti-entourage $\rho$ of $X$ induces a relation on the function set $X^A$ defined by
  \[ f \aent\rho g \iff \exists a\in A, (f(a) \aent\rho g(a)). \]
  This gives a base for an anti-quasi-uniformity on $X^A$, called the \emph{anti-quasi-uniformity of uniform convergence}.
  It is symmetric or a quasi-uniform apartness if $X$ is, but in general we cannot prove it to be locally decomposable even if $X$ is, unless $A$ is finite.
\end{eg}
...
\begin{thm}\label{thm:qunif-aqunif}
  ...
  \item The fixed objects on both sides are exactly the \nn-stable ones, so we have induced equivalences between these.
    ...
    % $\QUnifnn \simeq \AQUnifnn$.
% quasi-uniform spaces are those such that for any entourage $\al$, there is an entourage $\be$ such that $\neg\neg\be\implies \al$.
%     The fixed anti-quasi-uniform spaces are those such that for any anti-entourage $\rho$, there is an entourage $\si$ such that $\neg\neg\rho \implies \si$.
...
\begin{eg}
  The functors of \cref{thm:unif-ord,thm:aunif-ord} commute with the functors $\anti$ and $\neigh$ from \cref{thm:qunif-aqunif,thm:ord-le-apt}.
\end{eg}

\begin{eg}
  If $X$ is an anti-quasi-uniform space, then the structures of uniform convergence from \cref{eg:unif-uconv,eg:aunif-uconv} satisfy
  \[ \neigh(X^A) = \neigh(X)^A, \]
  essentially since $\neg\exists = \forall\neg$.
  However, since $\neg\forall \neq \exists\neg$, the dual fails: if $X$ is a quasi-uniform space, then we cannot prove $\anti(X^A) = \anti(X)^A$.
  This is a uniform-space version of the observation in \cref{eg:linf} that the $L^\infty$-norm is best defined with lower real numbers.
\end{eg}
...
\begin{thm}\label{thm:pmet-unif}
  ...
  \item The composites $\PMetUnifu \to \QUnif \to \Preord$ and $\PMetUnifl \to \QUnifApt \to \QApt$, of the functors defined above with the right adjoints from \cref{thm:unif-ord,thm:aunif-ord}, are equal to the functors of \cref{sec:point-point}.
  \end{enumerate}
\end{thm}
\begin{proof}
  ...
  Functoriality and full-faithfulness in both cases is immediate from the definitions of uniform continuity in \cref{def:pmet-maps,def:pmetl-maps}.
  ...
  If instead $X$ is Dedekind, then given $(d,\ep)$ consider the entourage $(d,\hfep)$.
  Then for any $x,y$ we have $d(x,y)<\ep$ or $d(x,y)>\hfep$, hence $d(x,y)<\ep$ or $\neg(d(x,y)<\hfep)$.

  Finally, the statements about underlying (anti-)preorders follow by inspection of \cref{def:pmet-ord}.
\end{proof}

\begin{eg}
  If $X$ is an upper prometric space, then the Hausdorff \mbox{(quasi-)}\allowbreak uniformity from \cref{eg:unif-haus} is the underlying (quasi-)uniformity of the Hausdorff prometric from \cref{eg:haus}.
\end{eg}

\begin{eg}
  If $X$ is an upper prometric space, then the quasi-uniformity underlying the supremum prometric from \cref{eg:linf} is that of uniform convergence from \cref{eg:unif-uconv}.
  For if $\sup_{a\in A} d(f(a),g(a)) < \ep$, then there is an $\ep'<\ep$ with $d(f(a),g(a))\le \ep'<\ep$ for all $a$; and conversely, for any $\ep$, if $d(f(a),g(a)) <\hfep$ for all $a$ then $\sup_{a\in A} d(f(a),g(a)) < \ep$.

  Dually, if $X$ is a lower prometric space, then the quasi-uniform apartness underlying the supremum prometric from \cref{eg:linf} is that of uniform convergence from \cref{eg:aunif-uconv}.
  This is even easier, since for \emph{lower} reals we have $\sup_{a\in A} d(f(a),g(a)) > \ep$ if and only if there exists an $a\in A$ such that $d(f(a),g(a))>\ep$.
  But note that it would fail if we used the MacNeille or upper real supremum instead.
\end{eg}
...
The constructions of \cref{thm:pmet-unif,thm:pmet-aunif} give one pair of such embeddings, but different embeddings are obtained by composing them with the uniform coreflections from \cref{thm:pmet-coref}.
...
It is also tricky, constructively, to produce quasi-gauges rather than prometrics, due in part to the failure of every uniform space to be completely regular (see \cref{sec:regularity}).
...
All three make sense in any kind of prometric space, but neighborhoods are best-behaved for upper prometrics, apartness is best-behaved for lower prometrics, and closeness is not constructively well-behaved at all.
To be more precise, $\approx$ is certainly reasonable to study, but it doesn't seem possible to axiomatize it in a way that captures \emph{all} the desired topological information; its natural ``binary additivity'' axiom is not constructively provable.
Thus, we will say little more about $\approx$, and proceed to axiomatize the basic properties of $\ll$ and $\bowtie$.
...
In contrast to \cref{sec:point-point,sec:uniformity}, here we will also consider structures that lack transitivity.
...
\begin{rmk}
TODO: The terminology ``apartness relation'' for an $\apart$ satisfying the strong disjunctive comparison axiom, especially if extended to ``quasi-apartness'' and ``quasi-uniform apartness space'', suggests that a ``topological apartness space'' should satisfy some similar disjunctive condition, without which these should be called something like an ``anti-topological space''.
Indeed,~\cite{bridges-vita} require an ``apartness space'' to satisfy ``weak local decomposability''.
Arguably full local decomposability would be an even better analogue.
Should we do this?
\end{rmk}
...
\begin{thm}\label{thm:top-topconcrete}
  ...
  \end{enumerate}
  Moreover, their subcategories $\Top$, $\LATop$, $\RATop$, %$\PTopnn$,
  $\ldTop$, and $\ldATop$ % (but not $\PATopnn$)
...
\begin{proof}
  ...
  % (Note that this construction does also work for \nn-stability, so that $\PATopnn$ is at least a fibration over $\Set$.)
...
Note, though, that in this case the two functors $\anti$ and $\neigh$ each only behave well with respect to \emph{one} of the weak transitivity axioms for topological apartness: $\anti$ produces left transitivity from neighborhood transitivity, while $\neigh$ produces neighborhood transitivity from \emph{right} transitivity.
Thus, to get an adjunction we need to generalize to pretopological spaces or restrict to locally decomposable ones (obtaining an equivalence in the latter case).
...
\begin{rmk}
  ...
  % \cref{thm:top-ll-bowtie} explains why \cref{thm:top-topconcrete} does not include $\PATopnn$: rather than being reflective in $\PATop$, the category $\PATopnn$ is \emph{coreflective} therein.
  % This implies that it is also topological over $\Set$ in its own right, though it is not closed under initial lifts in $\PATop$.
\end{rmk}
...
\begin{eg}\label{eg:aunif-topgp}
  This seems an appropriate place to put the following example.
  Recall from \cref{eg:unif-topgp} that, just as in classical mathematics, any topological neighborhood space with a compatible group structure admits a canonical uniform structure (in fact, two of them, obtained by switching the order of multiplication in $G$).
  Now suppose instead $G$ is a pretopological \emph{apartness} space with a compatible group structure.
  For each subset $A\subseteq G$ such that $e\bowtie A$, define 
  \[ x \aent A y \iff x\inv y \in A .\]
  These basic anti-entourages clearly satisfy irreflexivity (since $e\notin A$) as well as nullary and binary filtration (by nullary and binary additivity for $\bowtie$).
  Moreover, since inversion $G\to G$ is continuous, we have $e\bowtie A\inv$ if and only if $e\bowtie A$, giving symmetry; thus they form a base for a pre-anti-uniformity.

  To prove transitivity, we first note that by the construction in \cref{thm:top-topconcrete} of products in $\PATop$, in $X\times Y$ we have $(x,y) \bowtie C$ if and only if there exist $A\subseteq X$ and $B\subseteq Y$ such that $C \subseteq (A\times Y)\cup (X\times B)$ while $x\bowtie A$ and $y\bowtie B$.
  Thus, continuity of multiplication $m:G\times G\to G$ implies that if $e\bowtie C\subseteq G$, then $f\inv(C) \subseteq (A\times G)\cup (G\times B)$ where $e\bowtie A$ and $e\bowtie B$.
  Letting $D = A\cup B$, we have $f\inv(C) \subseteq (D\times G)\cup (G\times D)$ while $e\bowtie D$.

  We claim that $\aent D$ satisfies the uniform comparison axiom for $\aent C$.
  Thus, suppose $x \aent C z$, i.e.\ $x\inv z \in C$.
  Since $x\inv z = (x\inv y) (y\inv z)$, we have $(x\inv y, y\inv z) \in f\inv (C)$, and hence either $x\inv y \in D$ or $y\inv z \in D$.
  But in the former case $x\aent D y$, and in the latter case $y\aent D z$.
  
  Thus, any ``pretopological apartness group'' is automatically a ``uniform apartness group'' (and in two ways, the one described above and its dual using $x y\inv$).
  Moreover, the underlying topological apartness of this uniform apartness space is the original one.
  By definition, $x\bowtie A$ in the former means that there is a set $B$ such that $e\bowtie B$ and $\forall y, (y\in A \implies x\aent B y)$, and the latter is to say $A\subseteq x B$.
  But $e\bowtie B$ implies $x \bowtie x B$ by continuity of multiplication, hence $A\subseteq x B$ implies $x\bowtie A$ by isotony; and conversely, if $x\bowtie A$, we can take $B = x\inv A$.

  Note that right transitivity of the original structure comes for free, by \cref{thm:aunif-atop}.
  This is not anything special about apartness: a pretopological neighborhood group is automatically topological as well.

  Note that if $G$ is topologically locally decomposable, then it is also uniformly so; and in that case, this construction agrees with the standard one for topological neighborhood spaces and uniform spaces mentioned in \cref{eg:unif-topgp}.
  However, there are also interesting non-locally-decomposable examples.
  For instance, if $X$ is a lower-real-valued normed space, such as the $L^\infty$-norm on a function-space from \cref{eg:linf}, this shows that its uniform apartness structure is determined by its underlying topological apartness and its (abelian) group structure.
\end{eg}
...
\begin{eg}\label{eg:alexandrov}
  ...
  % TODO: Something about the $\oapt$-topology defined using a $\vee$, refer forwards to localic perspective.
\end{eg}
...
If $\apart$ coincides with $\oapt$, then this axiom becomes the statement that in the proof of \cref{thm:top-ord} we called \emph{weak local decomposability}.
...
We have seen that it does preserve \emph{right} transitivity, and moreover that right transitivity rather than left transitivity is what arises from lower prometrics; but $\anti$ doesn't yield right transitivity, so in any case there is a mismatch.

Instead of $\neigh$,~\cite{bridges-vita} use the following construction, which they call the \emph{apartness topology}:
...
\section{Interlude: Regularity and complete regularity}
\label{sec:regularity}

We define these only for neighborhood spaces, since as we will see below they both imply local decomposability.
...
\end{thm}
\begin{proof}
  TODO: Write in the uniform case; what follows is the Dedekind prometric space.
...
Of course, \cref{thm:sympmet-reg,thm:symgauge-creg} do depend crucially on symmetry; we have seen that every topological neighborhood space can be given a quasi-uniform structure and hence an (upper) prometric.
...
As in the topological case, the relation $\approx$ is interesting but not constructively axiomatizable to capture all the desired information.
Thus, we say little more about it, and proceed to axiomatize the relations $\ll$ and $\bowtie$.
...
% In fact, there may not even be ``enough'' Cauchy filters, so that the completion is properly a non-spatial locale.
