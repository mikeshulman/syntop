...
\begin{rmk}
  Our definition of topological apartness space is inspired by the point-set apartness spaces of Bridges and Vita\fxerror{cite book}, but their definition differs from ours in a couple of ways.
  ...
  Their $\apart$-transitivity, however, seems too weak even in classical mathematics, if $x\apart y$ means $x\oapt y$, i.e.\ $x\bowtie \singleton{y}$: when translated to $\ll$ it says that if $x\ll A$, then $x \ll \setof{ y | \forall z, (y\le z \implies z\in A) }$ which is a larger set that $\setof{y|y\ll A}$.\fxnote{Concrete counterexample?}
  Moreover, it seems insufficient for our proof of \cref{thm:top-topconcrete}\fxwarning{Is that true?}, though an alternative proof might be possible.
\end{rmk}
...
\begin{thm}\label{thm:pmet-unif}
  The definitions of \cref{def:pmet-unif} form bases for a locally decomposable quasi-uniformity and a locally decomposable anti-quasi-uniformity on any prometric space $X$, which are symmetric if $X$ is.
\end{thm}
\begin{proof}
