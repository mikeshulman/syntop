...
\begin{eg}
  The functors of \cref{thm:unif-ord} and \cref{thm:aunif-ord} commute with the functors $\anti$ and $\neigh$ from \cref{thm:qunif-aqunif} and \cref{thm:ord-le-apt}.
\end{eg}

\begin{eg}
  If $X$ is an anti-quasi-uniform space, then the structures of uniform convergence from \cref{eg:unif-uconv} and \cref{eg:aunif-uconv} satisfy
  \[ \neigh(X^A) = \neigh(X)^A, \]
  essentially since $\neg\exists = \forall\neg$.
...
\begin{thm}\label{thm:pmet-unif}
  ...
  \item The composites $\PMetUnifu \to \QUnif \to \Preord$ and $\PMetUnifl \to \QUnifApt \to \QApt$, of the functors defined above with the right adjoints from \cref{thm:unif-ord} and \cref{thm:aunif-ord}, are equal to the functors of \cref{sec:point-point}.
  \end{enumerate}
\end{thm}
\begin{proof}
...
\begin{eg}
  If $X$ is an upper prometric space, then the Hausdorff (quasi-)uniformity from \cref{eg:unif-haus} is the underlying (quasi-)uniformity of the Hausdorff prometric from \cref{eg:haus}.
\end{eg}
