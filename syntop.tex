...
The differences between the various kinds of real numbers, when used as distances, are illustrated by the following example.\fxnote{This is a special case of \cref{thm:preord-pmetu} and \cref{thm:qapt-pmetl}; is it worthwhile to repeat it here for expositional emphasis?}
...
\begin{defn}
  ...
  \item $\forall \rho \exists \si \forall xyz, (\neg(x\aent\si y) \land \neg(y\aent\si z) \implies \neg(x\aent\rho z))$ (weak transitivity).
        %For any anti-entourage $\rho$, there exists an anti-entourage $\si$ such that for any $x,y,z\in X$, if $\neg(x\aent\si y)$ and $\neg(y\aent\si z)$ then $\neg(x\aent\rho z)$ (weak transitivity).
  \end{enumerate}
  ...
  We have categories $\LAQUnif,\RAQUnif,\QUnifApt$, and $\ldAQUnif$.
\end{defn}
...
%     We call these objects \textbf{\nn-stable}.
  ...
  \item Since locally decomposable (anti)-quasi-uniform spaces are \nn-stable, we have an induced equivalence $\ldQUnif \simeq \ldAQUnif$.
...
Note, though, that a given quasi-uniform (apartness) space can be represented by many different prometrics (i.e.\ non-isomorphic objects of $\PMet$), and indeed there is more than one full embedding $\QUnif \to \PMetu$ or $\QUnifApt \to \PMetl$.
The constructions of \cref{thm:pmet-unif} and \cref{thm:pmet-aunif} give two such embeddings, but different embeddings are obtained by composing them with the uniform coreflections from \cref{thm:pmet-coref}.
...
\begin{defn}
  ...
  We write $\PATop$ for the category of pretopological apartness spaces and continuous maps, and $\LATop$, $\RTop$, $\PATopnn$, and $\ldATop$ for the above subcategories.
\end{defn}
...
\begin{thm}\label{thm:top-topconcrete}
  The categories $\PTop$ and $\PATop$ are ``topological over $\Set$'', i.e.\ their forgetful functor to $\Set$ has initial lifts for all sources and (hence) final lifts for all sinks.
  In particular:
  \begin{enumerate}
...
\begin{defn}
  A function $f:X\to Y$ between quasi-uniform spaces is \textbf{continuous} if for any point $x\in X$ and any entourage $\al$ of $Y$, there is an entourage $\be$ of $X$ such that $\forall y\in X, (x\ent\be y\implies f(x) \ent\al f(y))$.
  This defines a category $\QUnifTop$.
\end{defn}
...
\begin{defn}
  A function $f:X\to Y$ between anti-quasi-uniform spaces is \textbf{anti-continuous} if for any point $x\in X$ and any anti-entourage $\rho$ of $Y$, there is an anti-entourage $\si$ of $X$ such that $\forall y\in X, (f(x)\aent\rho f(y) \implies x\aent\si y)$.
  This defines categories such as $\RAQUnifTop$ and $\QUnifAptTop$.
\end{defn}
...
It does not seem possible to prove topological left transitivity from anti-uniform left transitivity, or even anti-uniform comparison.
However, anti-uniform local decomposability does suffice:

\begin{thm}
  If $X$ is a locally decomposable (anti-)uniform space, then its underlying topological structures are also locally decomposable, and the above functors commute with the equivalences.
  (TODO: Phrase better)
\end{thm}
\begin{proof}
  TODO.
  Also, to what extent do the above functors commute with the adjunctions in general?
\end{proof}

TODO: a converse embedding, and also the fine coreflection.  Relation to prometrics.
...
\begin{thm}
  ...
  Then the constructions of \cref{thm:top-ord} generalize to functors $\PTop \to \mathbf{Refl}$ and $\PATop \to \mathbf{Irref}$, and we have a mate-pair of natural transformations lying over the identity in $\Set$:
  \[
  \xymatrix{ \PTop \ar[r] \ar[d]_{\anti} \drtwocell\omit & \mathbf{Refl} \ar[d]^{\anti} \\
    \PATop \ar[r]  & \mathbf{Irref}  }
  \qquad
...
\begin{proof}
  ...
  Thus we have a functor $\PATop \to \mathbf{Irref}$.
...
In fact, the functors $\PTop \to \mathbf{Refl}$ and $\PATop \to \mathbf{Irref}$ defined above have left adjoints:
...
\begin{eg}\label{eg:alexandrov}
  ...
  This defines a left adjoint to the functor $\PATop \to \mathbf{Irref}$, and the apartness spaces obtained in this way are precisely those in which $x\bowtie A_i$ for all $i$ implies $x\bowtie \bigcup_i A_i$.
