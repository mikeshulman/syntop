...
Of course, any pseudo-metric space (of any type) defines a gauge space (of the same type) with exactly one gauging distance.
...
\begin{defn}
  ...
  \end{enumerate}
  We write $\PMetTopu$, $\PMetProxu$, $\PMetUnifu$, $\PMetAppu$, and $\PMetu$ for the categories of upper prometric spaces equipped with these five kinds of morphism, respectively.
...
\begin{thm}
  ...
  That is, we have
  \begin{center}
    short $\implies$ approach $\implies$ uniform $\implies$ proximal $\implies$ continuous.
  \end{center}
\end{thm}
\begin{proof}
  ...
  And finally, for proximal $\implies$ continuous, let $A = \singleton{x}$.
\end{proof}
...
\end{thm}
\begin{proof}
  All the proofs are essentially the same, except for approach $\implies$ uniform.
  Once again from $d_Y$ and $\ep$ we get a $d_X$ such that $\min(d_Y(f(x),f(y)),k) \le d_X(x,y) +\hfep$, where $k>\ep$, and we take $\de=\hfep$.
...
\begin{thm}\label{thm:anti-continuous}
  For MacNeille prometric spaces $X$ and $Y$, a function $f:X\to Y$ is continuous or uniformly continuous in the upper sense if and only if it is so in the lower sense.
  And if $f$ is proximally continuous in the upper sense, it is also so in the lower sense.
\end{thm}
\begin{proof}
...
However, proximal continuity in the lower sense seems weaker than in the upper sense, since $\neg\forall \not\implies \exists\neg$.
If necessary to disambiguate, we will refer to proximal continuity in the lower sense as \textbf{proximal anti-continuity}.


\section{Subcategories of prometric spaces}
...
From a category-theoretic perspective, however, one might argue that this removal of information is already accomplished by the definitions of weaker kinds of maps.
That is, one might reasonably \emph{define} an ``upper topological space'', say, to be an upper prometric space \emph{regarded as an object of} $\PMetTopu$, and the functor $\PMetu \to \PMetTopu$ (which is bijective on objects and faithful) as assigning the \emph{underlying topological space} of a prometric space.

However, this removal is not visible at the level of objects; hard to detect isomorphism.

Coreflective subcategories.

Still too much flab: a topological prometric, for instance, is an immense object.
