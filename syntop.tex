...
%   The categories of preorders, anti-preorders, and comparison anti-preorders are ``topological over $\mathbf{Set}$'', i.e.\ their forgetful functor to $\mathbf{Set}$ has initial lifts for all sources and (hence) final lifts for all sinks.
...
\begin{defn}
  ...
  We write $\PTop$ for the category of pretopological neighborhood spaces and continuous maps.
\end{defn}
...
\begin{defn}
  ...
  We write $\APTop$ for the category of pretopological apartness spaces and continuous maps.
\end{defn}
...
As we will see below, the relation $\approx$ is not very useful to axiomatize constructively, so we will not bother with all the constructive variants.
...
We will say no more about closure spaces.
However, topological neighborhood spaces and topological apartness spaces are both reasonable constructive notions of ``space''.
...
As for the relationship between neighborhood and apartness spaces, we have the following partial analogue of \cref{thm:ord-le-apt}:
...
A natural condition that ensures this is (local decomp)
...
\begin{rmk}
  ...
  However, it is unclear how to construct an ``Alexandrov apartness'' from an anti-preorder $\oapt$, even the comparison property $x\oapt y \lor y\oapt z$ seems insufficient, constructively, to ensure transitivity of any $\bowtie$ defined naturally from it.
