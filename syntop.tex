...
\begin{defn}\ \fxnote{cite Bridges}
  ...
  \item A topological neighborhood space $X$ is \textbf{locally decomposable} if whenever $x\ll A$, there is a subset $B$ such that $x\ll B$ and $\forall y, (y \ll A \lor y\not\ll B)$.\fxerror{Should be $x\notin B$; fix later}
  \end{enumerate}
\end{defn}

Note that local decomposability (for both sorts of space) is actually a strengthening of transitivity.
The assumption $\forall y, (y\in B \lor y\bowtie A)$ gives $\setof{y|y\not\bowtie A} \subseteq B$, while the assumption $\forall y, (y \ll A \lor y\not\ll B)$ gives $B\subseteq \setof{y|y\ll A}$, so in both cases transitivity follows from isotony.
...
\begin{proof}
  ...
  Suppose first that $\ll$ is locally decomposable and $x \bowtie A$, so that $x\ll \cpl{A}$; then we have a $B$ with $x\ll B$ and $\forall y, (y\ll \cpl{A} \lor y\not\ll B)$.
  Let $C = \cpl{\int(B)}$; then $x\ll \int(B)$ by transitivity, so $x\ll \cpl{C}$ by isotony, i.e.\ $x\bowtie C$.
  Moreover, any $y$ satisfies either $y\ll\cpl{A}$, i.e.\ $y\bowtie A$, or $y\not\ll B$, hence $y\notin \int(B)$, i.e.\ $y\in C$.
  ...
  Conversely, if $x\ll A$ then we have a $C$ with $x\ll C$ and $\forall y, (y\ll A \lor y\not\ll C)$, since $\ll$ is locally decomposable.
  Let $B = \cpl{\int(C)}$; then $x\ll \int(C)$ by transitivity, so $x\ll \cpl{B} = \cpl{\cpl{\int(C)}}$ by isotony.
  And any $y$ satisfies either $y\ll A$ (hence $y\in A$) or $y\not\ll C$ (hence $y\in B$).
\end{proof}
...
\begin{proof}
  ...
  By local decomposability we have a $B$ with \fxerror{finish}
\end{proof}
