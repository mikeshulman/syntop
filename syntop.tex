...
The differences between the various kinds of real numbers, when used as distances, are illustrated by the following example.\fxnote{This is a special case of \cref{thm:preord-pmetu} and \cref{thm:qapt-pmetl}; is it worthwhile to repeat it here for pedagogical emphasis?}
...
\begin{defn}\label{def:anti-preorder}
  A \textbf{weak anti-preorder} is a binary relation $\oapt$ on a set $X$ such that:
  \begin{enumerate}
  \item $\neg(x\oapt x)$ for all $x$ (irreflexivity).
  \item If $\neg(x\oapt y)$ and $\neg(y\oapt z)$, then $\neg(x\oapt z)$ (weak transitivity).
  \end{enumerate}
  a weak anti-preorder is\dots
  \begin{enumerate}[resume]
  \item a \textbf{left anti-preorder} if whenever $x\oapt z$ and $\neg(y\oapt z)$ we have $(x\oapt y)$ (left transitivity).
  \item a \textbf{right anti-preorder} if whenever $x\oapt z$ and $\neg(x\oapt y)$ we have $(y\oapt z)$ (right transitivity).
...
Note that comparison implies both left and right transitivity, which in turn each imply weak transitivity.
...
\begin{defn}
  ...
  \item If $X$ and $Y$ are anti-preorders, then $f$ is \textbf{monotone}, or \textbf{strongly extensional}, if $f(x)\oapt f(y) \implies x\oapt y$ for all $x,y\in X$.
    We write $\WAPreord$, $\LAPreord$, $\RAPreord$, and $\QApt$ for the categories of weak, left, and right anti-preorders or quasi-apartnesses, and monotone maps.
  \end{enumerate}
\end{defn}

\begin{thm}\label{thm:ord-le-apt}
  If $X$ is a preorder, then the relation $\neg(x\leapx y)$ is a left and right anti-preorder $\anti(X)$.
  Dually, if $X$ is a weak anti-preorder, then the relation $\neg(x\oapt y)$ is a preorder $\neigh(X)$.
  These operations define an idempotent adjunction $\anti\adj\neigh$:
  \[\anti : \Preord \rightleftarrows \WAPreord : \neigh\]
\end{thm}
\begin{proof}
  ...
  On the other hand, weak transitivity of an anti-preorder is precisely ordinary transitivity of its dual.
...
\begin{defn}
  A \textbf{weak anti-quasi-uniform space} is a set $X$ together with a family of binary relations, called \textbf{anti-entourages}, satisfying the following axioms.
  \begin{enumerate}
  ...
  \end{enumerate}
  A weak anti-quasi-uniform space is\dots
  \begin{enumerate}[resume]
  \item \textbf{left} (resp.\ \textbf{right}) anti-quasi-uniform if
    \begin{align*}
  ...
  \item a \textbf{quasi-uniform apartness space} if
    \fxwarning{Is this good terminology?  It generalizes the posetal case, but doesn't generalize to the topological case, unless we make it do so.}
  ...
  We have categories $\WAQUnif,\LAQUnif,\RAQUnif,\QUnifApt,\AQUnifnn$, and $\ldAQUnif$.
\end{defn}
